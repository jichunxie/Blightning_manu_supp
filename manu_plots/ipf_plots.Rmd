...
library('cluster')
library('dplyr')
library('patchwork')#,lib.loc = "/admin/apps/rhel8/R-4.1.1/lib64/R/library")
library('stats')
library('Rcpp')
library('reticulate')#,lib.loc = "/admin/apps/rhel8/R-4.1.1/lib64/R/library")
library('MASS')
#library('corpcor')
library('RcppArmadillo')#,lib.loc = "/admin/apps/rhel8/R-4.1.1/lib64/R/library")
library('MAST')
library('GSVA')
library(ggplot2)
...
validation_data = readRDS("~/Misc/B-Lightning/ipf/validation_data_matrix.rds")
validation_genename = readRDS("~/Misc/B-Lightning/ipf/validation_genename.rds") 
validation_cellinfo = readRDS("~/Misc/B-Lightning/ipf/validation_cellinfo.rds")
ciliated_so <- readRDS("~/Misc/B-Lightning/ipf/ciliated_so.rds")
```


```{r Fig.3a Log fold change Known Genes}

ret_ciliated <- readRDS("~/Misc/B-Lightning/ipf/ciliated_BLightning_results_CFS1_cutoff8.rds")


valid = readRDS('~/Misc/B-Lightning/ipf/Qi_summaryDt_new_MASTRE.rds')
...
df.logfc$group[df.logfc$coef<0] = "Downregulated"
df.logfc$group[df.logfc$coef>0] = "Upregulated"
df.logfc$pred[df.logfc$name %in% ret_ciliated[[4]]] = "Upregulated"
df.logfc$pred[df.logfc$name %in% ret_ciliated[[5]]] = "Downregulated"
...
#df.logfc$name[df.logfc$`Pr(>Chisq)` < 0.01]= paste(df.logfc$name,"***")
#df.logfc$name[df.logfc$`Pr(>Chisq)` > 0.01]= paste(df.logfc$name,"**")
...
ggplot(df.logfc, aes(fill=group, y=coef,x = as.factor(name))) + 
  ...
  xlab("Genes") +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size = 20,face = "bold"),panel.background = element_rect(fill = "white", colour = "grey50"),
    ...
    plot.title = element_text(size = 22, face = "bold")) + scale_fill_manual("Expression Level",values = c("#F8766D","#00aedb"),breaks=c('Upregulated', 'Downregulated')) + scale_y_continuous(limits=c(-2,6)) + ggtitle("Log Fold Change of Input Gene Markers")
dev.off()
```

```{r FIg.3b log fold change New genes}
df.logfc = logfc %>% filter(,name %in% c(ret_ciliated[[2]],ret_ciliated[[3]]))
df.logfc$group[df.logfc$coef<0] = "Downregulated"
df.logfc$group[df.logfc$coef>0] = "Upregulated"
df.logfc$pred[df.logfc$name %in% ret_ciliated[[2]]] = "Upregulated"
df.logfc$pred[df.logfc$name %in% ret_ciliated[[3]]] = "Downregulated"
df.logfc = df.logfc[order(df.logfc$group),]
...
ggplot(df.logfc %>% filter(verified == T), aes(fill=group, y=coef,x = as.factor(name))) + 
  ...
  xlab("Genes") +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size = 20,face = "bold"),panel.background = element_rect(fill = "white", colour = "grey50"),
                         ...
                         legend.title=element_text(size=20,face = "bold"), 
    legend.text=element_text(size=18,face = "bold"),
    plot.title = element_text(size = 22, face = "bold")) + scale_fill_manual("Expression Level",values = c("#F8766D","#00aedb"),breaks=c('Upregulated', 'Downregulated')) + scale_y_continuous(limits=c(-2,6)) +  ggtitle("Log Fold Change of Verified New Gene Markers")
dev.off()
```
...
save(ciliated_df,file = "~/ciliated_verified_df.RData")

```

```{r}
old_ret_ciliated = Markers

CFS1_0 = getscore("CFS1",old_ret_ciliated[[2]],old_ret_ciliated[[4]],
                        ciliated_so@assays$RNA@counts[c(old_ret_ciliated[[2]],old_ret_ciliated[[4]]),])

CFS1_1 = getscore("CFS1",old_ret_ciliated[[1]],old_ret_ciliated[[3]],
                        ciliated_so@assays$RNA@counts[c(old_ret_ciliated[[1]],old_ret_ciliated[[3]]),])

CFS1_2 = getscore("CFS1",c(old_ret_ciliated[[1]],old_ret_ciliated[[2]]),c(old_ret_ciliated[[3]],old_ret_ciliated[[4]]),
                        ciliated_so@assays$RNA@counts[c(old_ret_ciliated[[1]],old_ret_ciliated[[2]],old_ret_ciliated[[3]],old_ret_ciliated[[4]]),])

old_ciliated_df = as.data.frame(ciliated_so@meta.data)
old_ciliated_df$CFS1_0 = CFS1_0
old_ciliated_df$CFS1_1 = CFS1_1
old_ciliated_df$CFS1_2 = CFS1_2
save(old_ciliated_df,file = "~/old_ciliated_df.RData")
```
